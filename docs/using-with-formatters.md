...
- If you prefer to keep `---`, set yamlfmt's `formatter.include_document_start: true` in
  its `.yamlfmt` config and change ryl to `[rules.document-start]` `present = true`. The
...
YAML-specific options; the general [options](https://prettier.io/docs/options) that affect
YAML output are `printWidth`, `bracketSpacing` (the flow-mapping padding this recipe
relies on), `singleQuote`, `proseWrap`, and `endOfLine`.
...
[rules.document-start]
present = false             # Prettier never adds `---`
...
|---|---|---|---|
| `document-start` `present` | `false` | either | `true` |
| `document-end` `present` | off | either | off |
