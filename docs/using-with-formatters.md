...
- The flow-padding split is the key alignment: `braces` must allow one inner space, while
  ...
  `max-spaces-inside-empty = 0`; without those, ryl rewrites `{}` to `{ }` and Prettier
  reverts it, an edit loop. Setting either `braces` or `brackets` to `forbid` clashes
...
- `quoted-strings` must not require single quotes: Prettier normalizes to double quotes
  by default (`singleQuote: false`), so `quote-type = "single"` loops; `only-when-needed`
  (or `double`) is safe. If you set Prettier `singleQuote: true` (it does affect YAML),
  flip to `quote-type = "single"` instead.
